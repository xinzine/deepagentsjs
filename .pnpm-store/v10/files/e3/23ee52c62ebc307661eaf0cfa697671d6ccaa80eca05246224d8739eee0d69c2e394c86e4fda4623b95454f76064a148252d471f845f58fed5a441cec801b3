...
const inputSchema = z.object({
	...
	extractDepth: z.enum(["basic", "advanced"]).optional().describe(`Controls the thoroughness of web content extraction.
...
Better for complex websites but may increase response time.`),
	includeImages: z.boolean().optional().describe(`Determines whether to extract and include images from the source URLs.
...
Default is False (extracts text content only).`),
	query: z.string().optional().describe("User intent query for reranking extracted content chunks.")
});
var TavilyExtract = class extends StructuredTool {
	static lc_name() {
...
//#endregion
export { TavilyExtract };
//# sourceMappingURL=tavily-extract.js.map