...
class SandboxPythonCodeToolbox {
    getRunCommand(code, params) {
        ...
        let base64Code = buffer_1.Buffer.from(code).toString('base64');
        // Override plt.show() method if matplotlib is imported
        if (SandboxPythonCodeToolbox.isMatplotlibImported(code)) {
            let code_wrapper = buffer_1.Buffer.from(PYTHON_CODE_WRAPPER, 'base64').toString('utf-8');
            code_wrapper = code_wrapper.replace('{encoded_code}', base64Code);
            base64Code = buffer_1.Buffer.from(code_wrapper).toString('base64');
        }
...
`;
//# sourceMappingURL=SandboxPythonCodeToolbox.js.map