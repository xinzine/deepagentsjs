...
const inputSchema = z.object({
	...
	includeDomains: z.array(z.string()).optional().describe(`A list of domains to restrict search results to.
...
Default is None (no domain restriction).`),
	excludeDomains: z.array(z.string()).optional().describe(`A list of domains to exclude from search results.
...
Default is None (no domain exclusion).`),
	searchDepth: z.enum(["basic", "advanced"]).optional().describe(`Controls search thoroughness and result comprehensiveness.
...
rare information, or when in-depth analysis is needed.`),
	includeImages: z.boolean().optional().describe(`Determines if the search returns relevant images along with text results.
...
* ```
*/
var TavilySearch = class extends StructuredTool {
	static lc_name() {
		return "TavilySearch";
	}
