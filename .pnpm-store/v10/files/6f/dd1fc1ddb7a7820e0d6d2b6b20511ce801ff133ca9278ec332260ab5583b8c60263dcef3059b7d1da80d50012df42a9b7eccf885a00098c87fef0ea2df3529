...
// Note: Warning is only emitted once per process, so in multi-test runs
// the warning may already have been triggered by an earlier test.
// We set up the expectation but don't require it to fire.
...
process.on('warning', () => {
  // Warning listener - not using mustCall since warning may already have fired
});
